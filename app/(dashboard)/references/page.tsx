        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {t.raw('items').map((item:any, index:any) => (
            <div key={index} className="relative bg-white shadow-lg rounded-lg overflow-hidden p-6 flex flex-col items-center justify-center transition transform hover:-translate-y-2 hover:shadow-2xl">
              <Image src={item.src} alt={item.alt} width={64} height={64} className="h-16 w-auto my-auto " />
              <p className="text-xl font-semibold mt-4">{item.name}</p>
              <div className="absolute bottom-0 right-0">
                <Image src="/cs_logo.png" alt="Çukurdere Sondaj" width={50} height={70} className="h-10 w-16 rounded-tl-lg" />
              </div>
            </div>
          ))}
        </div>
